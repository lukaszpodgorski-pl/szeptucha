...
fn default_post_process_providers() -> Vec<PostProcessProvider> {
    // Szeptucha is local-only: post-processing never reaches a cloud API. The
    // single provider points at any OpenAI-compatible local server (Ollama,
    // LM Studio, vLLM, llama.cpp). The base URL stays user-editable so it can be
    // repointed at an internal endpoint.
