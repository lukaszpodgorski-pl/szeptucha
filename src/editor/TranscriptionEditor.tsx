...
const TranscriptionEditor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // First open: the window may finish loading after the backend emitted
    // the event, so pull the pending text instead of relying on the race.
    commands.getPendingEditorText().then((pending) => {
      if (pending !== null) {
        setText(pending);
        requestAnimationFrame(() => ref.current?.focus());
      }
