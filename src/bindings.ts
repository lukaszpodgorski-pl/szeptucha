...
async changeLivePreviewSetting(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_live_preview_setting", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changeEditBeforePasteSetting(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_edit_before_paste_setting", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async pasteText(text: string) : Promise<void> {
    await TAURI_INVOKE("paste_text", { text });
},
async getPendingEditorText() : Promise<string | null> {
    return await TAURI_INVOKE("get_pending_editor_text");
},
async cancelTranscriptionEditor() : Promise<void> {
    await TAURI_INVOKE("cancel_transcription_editor");
},
